...
opt = HybridAdamBFGSOptimizer(warm_start = 0)
losses = Optimize!(sess,loss,opt, 100)
# cb(run(sess, [E, cH, loss1, loss2]), 101, run(sess, loss))
...
figure(figsize = (10,4))
subplot(121)
dif = abs.((@. 1/(xH^2+1) + 2.0) - run(sess, cH, θ=>opt.x))
plot(xH, dif, "-", label = "Difference")
legend()
subplot(122)
semilogy(losses)
savefig("results/cH_$mode$SEED.png")
using JLD2 
is_bfgs = opt.is_bfgs
@save "results/cH_$mode$SEED.jld2" losses xH dif is_bfgs